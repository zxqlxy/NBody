...
This will simulate two galaxies.
"""
import sys
sys.path.append(".")
...
if __name__ == '__main__':
    ...
    dim = args.dimension
    N = args.num_part  # how many particles?
    M_part = M_total / N  # mass of each particle (in 10^9 M_sun)
    ...
    if rank == 0:
        ...
        pos_init1, vel_init1 = initialConditions.plummer(N/2, dim, a, m=M_part,
                                                       G=GRAV_CONST, seed=seed)
        pos_init2, vel_init2 = initialConditions.plummer(N/2, dim, a, m=M_part,
                                                       G=GRAV_CONST, seed=seed)
        
        # move one of the away in x-axis
        # 10 seems to be a good start
        pos_init2[:, 0] = pos_init2[:, 0] + 10*a
        vel_init1[:, 0] = max(vel_init1[:, 0])
        vel_init2[:, 0] = -max(vel_init2[:, 0])
        pos_init = np.concatenate((pos_init1,pos_init2), axis = 0)
        vel_init = np.concatenate((vel_init1,vel_init2), axis = 0)

        # sanity check of shape
        assert(pos_init.shape == (N, dim))
        assert(vel_init.shape == (N, dim))
