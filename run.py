...
if __name__ == '__main__':
    ...
    pos = np.empty((N_this, 2))
    vel = np.empty((N_this, 2))
    comm.Scatterv([pos_init, N_per_process*2, displacements, MPI.DOUBLE],
                  pos, root=0)
    comm.Scatterv([vel_init, N_per_process*2, displacements, MPI.DOUBLE],
                  vel, root=0)
    # compute forces
    ...
    vel_full = None
    if rank == 0:
        pos_full = np.empty((N_parts, 2))
        vel_full = np.empty((N_parts, 2))
    ...
    for i in range(N_steps):
        ...
        pos = np.empty((N_this, 2))
        vel = np.empty((N_this, 2))
        comm.Scatterv([pos_full, N_per_process*2, displacements, MPI.DOUBLE],
                      pos, root=0)
        comm.Scatterv([vel_full, N_per_process*2, displacements, MPI.DOUBLE],
                      vel, root=0)
        if rank == 0:
